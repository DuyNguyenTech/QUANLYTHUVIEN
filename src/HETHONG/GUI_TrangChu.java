...
public class GUI_TrangChu extends JPanel {
    ...
    // Đảm bảo bạn đã import đúng class DTO_TaiKhoan
        ...
        new Thread(() -> {
            try {
                ...
                if (tk.getPhanQuyen() == 1 || tk.getPhanQuyen() == 2) {
                    ...
                    // Fix lỗi gạch đỏ: Đảm bảo class DTO_TaiKhoan có phương thức getMaDocGia()
                    String maDG = tk.getMaDocGia(); 
                    
                    // Gọi hàm từ DAL
                    dangMuon = dalThongKe.getCountGlobal("SELECT COUNT(*) FROM phieu_muon WHERE MaDocGia = '" + maDG + "' AND TinhTrang = N'Đang mượn'");
                    quaHan = dalThongKe.getCountGlobal("SELECT COUNT(*) FROM phieu_muon WHERE MaDocGia = '" + maDG + "' AND TinhTrang = N'Quá hạn'");
                ...
                // Final variables để dùng trong SwingUtilities
                final int s = sach, d = docgia, m = dangMuon, q = quaHan;
                final ArrayList<Object[]> activities = listFinal;
                final String nt = notice; 
                final ArrayList<Object[]> sg = suggest;

                SwingUtilities.invokeLater(() -> {
                    lblTotalSach.setText(String.valueOf(s));
                    if (tk.getPhanQuyen() == 1 || tk.getPhanQuyen() == 2) {
                        lblTotalDocGia.setText(String.valueOf(d));
                    } else {
                        lblTotalDocGia.setText("Cá nhân");
                    }
                    ...
                    if (tk.getPhanQuyen() == 1 || tk.getPhanQuyen() == 2) {
                        ...
                        if(txtThongBao != null) txtThongBao.setText(nt);
                        if(txtSuggest != null) {
...
//                
//             // Thay vì dùng SQL cứng trong GUI, hãy gọi hàm từ DAL để đảm bảo an toàn
//                if (tk.getPhanQuyen() == 1 || tk.getPhanQuyen() == 2) {
//                    docgia = dalThongKe.getTongDocGia();
//                    dangMuon = dalThongKe.getTongPhieuMuon();
//                    quaHan = dalThongKe.getTongViPham();
...
//                } else {
//                    String maDG = tk.getMaDocGia();
//                    // Sử dụng tham số N'...' để tránh lỗi font tiếng Việt khi so sánh TinhTrang
//                    dangMuon = dalThongKe.getCountGlobal("SELECT COUNT(*) FROM phieu_muon WHERE MaDocGia = '" + maDG + "' AND TinhTrang = N'Đang mượn'");
//                    quaHan = dalThongKe.getCountGlobal("SELECT COUNT(*) FROM phieu_muon WHERE MaDocGia = '" + maDG + "' AND TinhTrang = N'Quá hạn'");
//                    notice = dalThongKe.getThongBaoMoiNhat();
//                    suggest = dalThongKe.getSachGoiY(maDG);
